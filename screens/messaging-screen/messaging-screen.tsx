...
function MessageScreen(props: Props) {
  ...
  const validateChatInstance = async (
    ...
    if (instance.exists) {
      ...
      if(chat.reply){
        addMessagesToFirestore(chat.chat.reply)
      }
      messagesListener();
      console.log('chat exists, getting messages...');
    } else {
      createUserChatInstance(chat.chat.chatId);
  ...
  useEffect(() => {
    //console.log('ute')
    if(props.chat.chat.reply){
      addMessagesToFirestore(props.chat.chat.reply)
    }
    ...
    return () => {
      messagesListener(),
       initializeAudioRecorder(),
        validateChatInstance(props.chat.chat, messagesListener),
  ...
  const renderMessageImage = (props: any) => {
    const {currentMessage} = props;
    return (
      <Lightbox  backgroundColor={'white'}  renderContent={() => viewImage(props)}>
        <Image
  ...
  const viewImage = (props: any) => {
    return (
      <Image
        ...
        style={{height: 900, width: '100%', resizeMode: 'contain'}}
      />
    );
  };
        ...
        <View style={styles(props).chatHeaderItemsContainer}>
          ...
          <View style={styles(props).merchantNameContainer}>
            ...
            <Text style={[styles(props).messageText, {}]}>
              {props.chat.chat.merchant.name}
            </Text>
          </View>
          <Image source={{uri: props.chat.chat.merchant.photo}}
                 style={styles(props).merchantImageContainer}
            />
        </View>
        <View style={styles(props).onlineContainer}>
          {props.chat.chat.merchant.online ? (
            <React.Fragment>
            ...
            <React.Fragment>
              <Text style={styles(props).onlineText}>Offline </Text>
